  ...
  return (
    <div className='bg-[#3C3D37] flex flex-col justify-center h-screen w-full'>
      ...
      <div className='flex flex-col items-end justify-center h-[45vh]'>
        ...
        <div className='text-7xl text-white font-bold mt-[5vh] mr-[1rem]'>{ calculate() }</div>
      </div>
      <div className='grid grid-cols-4 place-items-center gap-2'>
        <button onClick={()=>{addOperator("+")}}  className='btn-operator'>
          +
        </button>
        <button onClick={()=>{addOperator("*")}}  className='btn-operator'>
          x
        </button>
        <button onClick={()=>{setPreview(preview.slice(0,-1))}} className='bg-[#ECDFCC] text-center text-3xl w-[80%] h-[8vh] rounded-lg mt-[1rem] mb-[1rem]'>
          DEL
        </button>
        <button onClick={()=>{setPreview("")}} className='bg-[#D45555] text-center text-white text-3xl w-[80%] h-[8vh] rounded-lg mt-[1rem] mb-[1rem]'>
          AC
        </button>
      </div>
